...
import { bulkImportMarketplaceOrders } from '@/features/sales/actions/marketplace-actions'
import { useQueryClient } from '@tanstack/react-query'

interface ImportMarketplaceOrdersModalProps {
    isOpen: boolean
    onClose: () => void
}
...
// Required CSS headers (flexible matching)
...
export function ImportMarketplaceOrdersModal({ isOpen, onClose }: ImportMarketplaceOrdersModalProps) {
    ...
    const analyzeFile = (file: File) => {
        ...
        Papa.parse(file, {
            ...
            complete: (results) => {
                ...
                // Simple validation: Ensure essential columns exist
                // We allow some flexibility in naming via logic in backend, but frontend check ensures basic structure
                const missingHeaders = REQUIRED_HEADERS.filter(h => {
                    // Check if header exists directly or likely mapping
                    return !headers.some(header =>
                        header.toLowerCase().includes(h.toLowerCase()) ||
                        (h === 'Date' && header.includes('Order Date')) ||
                        (h === 'Customer Name' && header.includes('Customer')) ||
                        (h === 'Product Name' && header.includes('Product'))
                    )
                })

                if (missingHeaders.length > 0 && missingHeaders.length === REQUIRED_HEADERS.length) {
                    // Only fail if ALL look missing (likely wrong file)
                    // If partial match, we proceed and let validation per row handle it
                    toast.error(`CSV might be missing required columns like: ${REQUIRED_HEADERS.join(', ')}`)
                }

                // Analyze rows
                const analyzed = results.data.map((row: any, index: number) => {
                    // Normalize keys for checking
                    const customer = row['Customer Name'] || row['Customer']
                    const product = row['Product Name'] || row['Product']
                    const amount = row['Amount'] || row['Price']

                    const isValid = customer?.trim() && product?.trim() && amount

                    return {
                        row: index + 1,
                        status: isValid ? 'valid' : 'error',
                        message: isValid ? 'Ready to import' : 'Missing Customer, Product or Amount',
                        data: row
                    } as ValidationResult
                })
    ...
    const handleImport = async () => {
        ...
        try {
            const result = await bulkImportMarketplaceOrders(validRows)
            ...
            if (result.success > 0) {
                ...
                queryClient.invalidateQueries({ queryKey: ['marketplace-orders-filtered'] })
            } else {
                        ...
                        <>
                            ...
                            {!file ? (
                                ...
                                >
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">Columns: Date, Customer, Phone, Product, Qty, Amount, Courier (Optional)</p>
                                    <input
                                        type="file"
                                        ref={fileInputRef}
                                        ...
                                        <table className="w-full text-left text-[10px]">
                                            <thead className="bg-gray-50 dark:bg-zinc-800 sticky top-0">
                                                <tr>
                                                    ...
                                                    <th className="px-2 py-1.5 font-medium">Courier</th>
                                            ...
                                            <tbody className="divide-y dark:divide-zinc-700">
                                                {previewData.slice(0, 100).map((row, idx) => (
                                                    <tr key={idx} className={row.status === 'valid' ? '' : 'bg-red-50 dark:bg-red-900/10'}>
                                                        ...
                                                        <td className="px-2 py-1">{row.data['Customer Name'] || row.data['Customer']}</td>
                                                        <td className="px-2 py-1">{row.data['Product Name'] || row.data['Product']}</td>
                                                        <td className="px-2 py-1">{row.data['Courier'] || '-'}</td>
                                                        <td className="px-2 py-1">
                                                            {row.status === 'valid' ? (
                                                                <span className="text-green-600 flex items-center gap-1"><CheckCircle size={10} /> Ok</span>
                                                            ) : (
