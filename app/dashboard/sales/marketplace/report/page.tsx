...
import {
    getMarketplaceSalesByFiscalYear,
    getMarketplaceMonthlySalesByFiscalYear,
    getMarketplaceDailySalesByFiscalYear
} from '@/features/sales/actions/marketplace-actions'
import { getAllFiscalYears, getActiveFiscalYear } from '@/features/sales/actions/daraz-actions'
import { ArrowLeft, TrendingUp, Package, DollarSign, List, Calendar } from 'lucide-react'
import Link from 'next/link'
import { Card } from '@/components/ui-shim'

export default function MarketplaceSalesReportPage() {
    ...
    const { data: salesSummary, isLoading: loadingSummary } = useQuery({
        queryKey: ['marketplace-sales-summary', selectedFiscalYear],
        queryFn: () => getMarketplaceSalesByFiscalYear(selectedFiscalYear),
        enabled: !!selectedFiscalYear
    })

    // Fetch monthly breakdown
    const { data: monthlyData, isLoading: loadingMonthly } = useQuery({
        queryKey: ['marketplace-monthly-sales', selectedFiscalYear],
        queryFn: () => getMarketplaceMonthlySalesByFiscalYear(selectedFiscalYear),
        enabled: !!selectedFiscalYear
    })

    // Fetch daily breakdown
    const { data: dailyData, isLoading: loadingDaily } = useQuery({
        queryKey: ['marketplace-daily-sales', selectedFiscalYear],
        queryFn: () => getMarketplaceDailySalesByFiscalYear(selectedFiscalYear),
        enabled: !!selectedFiscalYear
    })
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 dark:bg-zinc-900">
            ...
            <div className="sticky top-0 z-10 bg-white dark:bg-zinc-900 border-b dark:border-zinc-800 px-3 py-1.5 flex items-center justify-between shadow-sm">
                <div>
                    <h1 className="text-sm font-bold">Marketplace Sales Report</h1>
                    {selectedFY && (
                ...
                <div className="flex items-center gap-2">
                    <Link
                        href={`/dashboard/sales/marketplace/order-list${selectedFiscalYear ? `?fiscalYearId=${selectedFiscalYear}` : ''}`}
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
                {loadingSummary ? (
                    ...
                    <>
                        ...
                        {/* Daily Sales Table (Day by Day) */}
                        <Card className="dark:bg-zinc-900 dark:border-zinc-700">
                            <div className="p-3">
                                <h3 className="text-[11px] font-bold uppercase text-gray-900 dark:text-gray-100 mb-2 flex items-center gap-2">
                                    <Calendar size={14} />
                                    Day by Day Sales
                                </h3>
                                {loadingDaily ? (
                                    <div className="text-center py-4 text-xs text-gray-500">Loading daily data...</div>
                                ) : dailyData && dailyData.length > 0 ? (
                                    <div className="overflow-x-auto">
                                        <table className="w-full table-auto border-collapse">
                                            <thead className="bg-gray-50 dark:bg-zinc-800">
                                                <tr>
                                                    <th className="px-2 py-1 text-left text-[9px] font-bold uppercase text-gray-900 dark:text-gray-100">Date</th>
                                            ...
                                            <tbody className="divide-y divide-gray-200 dark:divide-zinc-700">
                                                {dailyData.map((day, idx) => (
                                                    <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                                                        <td className="px-2 py-1 text-[11px] text-gray-700 dark:text-gray-300">
                                                            {new Date(day.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}
                                                        </td>
                                                        <td className="px-2 py-1 text-right text-[11px] text-gray-700 dark:text-gray-300">{day.orderCount}</td>
                                                        <td className="px-2 py-1 text-right text-[11px] text-gray-700 dark:text-gray-300">{day.quantity}</td>
                                    ...
                                    <div className="text-center py-4 text-xs text-gray-500">No daily data available</div>
                                )}
                            </div>
                        </Card>
                    </>
