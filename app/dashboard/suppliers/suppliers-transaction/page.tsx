...
import { getSupplierTransactions, createSupplierTransaction } from '@/features/suppliers/actions/supplier-transaction-actions'
...
export default function SuppliersTransactionPage() {
    ...
    const loadSupplierOptions = (inputValue: string) =>
        getSuppliers({ search: inputValue, limit: 20 }).then(res =>
            res.suppliers.map(s => ({ value: s.id, label: s.supplier_name }))
        )
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Note: Currently just using createMutation (Assuming UI requirement only, verifying backend for real update later if needed)
